...
export default function registerP1() {
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      router.push("/Propietario/registerP2"); // Asegúrate de que la ruta sea correcta
    }
  };
  ...
  return (
    <View className="bg-[#C3B6E3] w-full h-full items-center justify-center">
      <View className="bg-white rounded-[33px] w-96 px-4 items-center py-10">
        ...
        <TextInput
          className={`h-14 w-full border ${errors.nombre ? 'border-red-500' : 'border-[#C4C4C4]'} rounded-xl p-3 font-outfit text-xl`}
          placeholder="Nombre(s)*"
          value={formData.nombre}
          onChangeText={(text) => handleNameChange(text, 'nombre')}
        />
        {errors.nombre && <Text className="text-red-500 text-sm">{errors.nombre}</Text>}
        
        <View className="flex flex-row my-2">
          <View className="flex-1">
            <TextInput
              className={`h-14 w-full border ${errors.apellidoPaterno ? 'border-red-500' : 'border-[#C4C4C4]'} rounded-xl p-3 font-outfit text-xl`}
              placeholder="Apellido Paterno*"
              value={formData.apellidoPaterno}
              onChangeText={(text) => handleNameChange(text, 'apellidoPaterno')}
            />
            {errors.apellidoPaterno && <Text className="text-red-500 text-sm">{errors.apellidoPaterno}</Text>}
          </View>
          <View className="flex-1">
            <TextInput
              className={`h-14 w-full border ${errors.apellidoMaterno ? 'border-red-500' : 'border-[#C4C4C4]'} rounded-xl p-3 font-outfit text-xl`}
              placeholder="Apellido Materno*"
              value={formData.apellidoMaterno}
              onChangeText={(text) => handleNameChange(text, 'apellidoMaterno')}
            />
            {errors.apellidoMaterno && <Text className="text-red-500 text-sm">{errors.apellidoMaterno}</Text>}
          </View>
        </View>
        
        <TextInput
          className={`h-14 w-full border ${errors.fechaNacimiento ? 'border-red-500' : 'border-[#C4C4C4]'} rounded-xl p-3 font-outfit text-xl`}
          placeholder="Fecha de Nacimiento (DD/MM/AAAA)*"
          value={formData.fechaNacimiento}
          onChangeText={(text) =>  setFormData({...formData, fechaNacimiento: text})}
        />
        {errors.fechaNacimiento && <Text className="text-red-500 text-sm">{errors.fechaNacimiento}</Text>}
